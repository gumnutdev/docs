...
```bash
npm install gumnut-v0-api@~0.2.9
# or
yarn add gumnut-v0-api@~0.2.9
```
...
After importing Gumnut at the top of `client.js`, call `connectToGumnutDoc` to tell Gumnut we want to start tracking this document. This methods takes two parameters:
- A document ID which Gumnut will join (or create if it doesn't exist) 
- A function which generates an authentication token
