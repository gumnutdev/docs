...
<Tip>Note that this is completely unstyled by default, so the example below includes a `border` quite literally so you can see where the component is!</Tip>
  ...
  return <>
    <GumnutData
      ...
      render={(arg) => (
        ...
        <!-- You can also render metadata here -->
        {arg.state.dirty ? <div>*Dirty</div> : ''}
        <div>Clients focused here => {arg.state.clients.join(',')}</div>
      )}
    />
  </>
}
