...
Gumnut uses the parameters passed to `buildTestToken` to attribute (eg associated) changes to a specific user. The first parameter is a unique ID (which is also shown to other users during editing.) The second parameter is an object whose properties are displayed in the Gumnut dashboard.

Once the token is provided, calling `gumnut.join` with the AbortController and a document ID will join that document (and create it, if it's new! Handy.) 
...
### 4. Create a node within the document
